...
import type { CronDelivery, CronJob, CronRunLogEntry, CronStatus } from "../types.ts";
...
export async function loadCronJobs(state: CronState) {
  ...
  try {
    const res = await state.client.request<CronListResponse>("cron.list", {
...
export function buildCronPayload(form: CronFormState) {
  ...
  if (form.deliveryMode === "announce") {
    ...
    payload.provider = form.deliveryChannel || "last";
  }
  if (form.deliveryTo.trim()) {
    payload.to = form.deliveryTo.trim();
  }
...
function buildCronDelivery(form: CronFormState): CronDelivery | undefined {
...
export async function addCronJob(state: CronState) {
  ...
  try {
    ...
    const job = {
      ...
      payload,
      delivery: buildCronDelivery(form),
      isolation:
...
export async function loadCronRuns(state: CronState, jobId: string) {
  ...
  try {
    const res = await state.client.request<CronRunsResponse>("cron.runs", {
      id: jobId,
      limit: 50,
    });
