...
export type UiVisualPreset = "neo-v2" | "neo-v1";

export type UiMotionLevel = "full" | "reduced";

export type ChatTimelineDensity = "summary" | "expanded";

export type ChatObservabilityPin = "timeline" | "insights";
...
export type ChatTimelineFilterState = {
  runId: string;
  streams: Record<string, boolean>;
};

export type ChatTimelineRunStatus = "running" | "success" | "error" | "aborted";

export type ChatTimelineRunSummary = {
  sessionKey: string;
  runId: string;
  startedAt: number;
  endedAt?: number;
  status: ChatTimelineRunStatus;
  totalMs?: number;
  firstTokenMs?: number;
  toolCalls: number;
  toolErrors: number;
...
export type ChatTimelinePanelState = {
  open: boolean;
  tab: "timeline" | "tool" | "insights";
  follow: boolean;
