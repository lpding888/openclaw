...
export function renderOverviewCards(props: OverviewCardsProps) {
  ...
  const cronValue =
    cronEnabled == null
      ? t("common.na")
      : cronEnabled
        ? t("overview.cards.jobsCount", { count: String(cronJobCount) })
        : t("common.disabled");

  const cronHint =
    failedCronCount > 0
      ? html`<span class="danger">${t("overview.cards.failedCount", { count: String(failedCronCount) })}</span>`
      : cronNext
  ...
  const cards: StatCard[] = [
    {
      ...
      hint: redact(
        t("overview.cards.tokenMessageHint", { tokens: totalTokens, messages: totalMessages }),
        props.redacted,
      ),
    ...
    },
    {
      ...
      hint:
        blockedSkills > 0
          ? t("overview.cards.blockedCount", { count: String(blockedSkills) })
          : t("overview.cards.activeCount", { count: String(enabledSkills) }),
    },
    {
      kind: "cron",
      tab: "cron",
